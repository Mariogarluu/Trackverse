-- Add external_id to Catalog Tables to prevent duplicates from external APIs
...
-- Function to Track an Item (Get or Create Catalog Entry -> Track for User)
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- run with privileges of creator to access tables if RLS is strict (though policies cover it)
AS $$
DECLARE
  v_catalog_id UUID;
